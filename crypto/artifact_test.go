...
func TestArtifactRoundTrip(t *testing.T) {
	...
	if artifact.Format != ArtifactFormat || artifact.Version != ArtifactVersion {
		t.Fatalf("unexpected artifact metadata: %#v", artifact)
	}
	if artifact.AccountID != "123456789012" || artifact.Region != "ap-south-1" {
		t.Fatalf("unexpected artifact identity: %#v", artifact)
	}
	if artifact.SnapshotHash != HashBytes(payload) {
		t.Fatal("artifact snapshot hash does not match payload")
	}
	...
	if err != nil {
		t.Fatalf("DeserializeArtifact: %v", err)
	}
...
func TestArtifactRejectsEmptyPayloadAndUnsupportedMetadata(t *testing.T) {
	if _, err := NewArtifact(nil, "account", "region", []byte("pass")); err == nil {
	...
	if _, err := DeserializeArtifact(data); err == nil {
		t.Fatal("expected unsupported artifact version to be rejected")
	}
}
